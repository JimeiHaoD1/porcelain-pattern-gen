(() => {
  ...
  const ALLOWED_SEEDS = [4101, 4102, 4103];
  ...
  const state = {
    seed: Number(window.EDITOR_SEED),
    bootstrap: null,
    bootstraps: new Map(),
    seedSlots: new Map(),
    branches: [],
  ...
  function handleCircle(point, className, type, details = {}, radius = 4.1) {
    const node = svgNode("circle", {
      cx: point[0], cy: point[1], r: radius, class: `handle ${className}`, "data-handle-type": type,
  ...
  function drawDirectControls(group, branch) {
    ...
    for (let segment = 0; segment < cubics.length - 1; segment += 1) {
      ...
      const diamond = svgNode("polygon", {
        points: `${join[0]},${join[1] - 5} ${join[0] + 5},${join[1]} ${join[0]},${join[1] + 5} ${join[0] - 5},${join[1]}`,
  ...
  function updateStatus() {
    const branch = selectedBranch();
    if (branch) {
      ui.selectionStatus.textContent = `${branch.curve_id} · L${branch.level} · 父枝 ${branch.parent_id} · mount ${branch.mount_fraction.toFixed(6)} · ${branch.status}`;
      ...
      ui.selectionStatus.textContent = `Seed ${state.seed}：${state.branches.length} 枝（L1 ${counts[0]} / L2 ${counts[1]} / L3 ${counts[2]}）`;
      ui.geometryStatus.textContent = "主干、花位锁定；所有区域均为 SVG 结构数据";
    }
  ...
  function updateUnitStats() {
    const branch = selectedBranch();
    if (!branch) {
      ui.unitStats.textContent = "选择任意分支查看所属 Unit";
      return;
    }
    ...
    ui.unitStats.textContent = `${topUnitId(branch.curve_id)}：minimum ${required}，maximum ${required + optional}；禁用 ${forbidden}；总槽位 ${members.length}`;
  }
  ...
  async function save() {
    ...
    setMessage("正在由 Python 重算几何并原子保存…");
    try {
      ...
      setMessage(`已保存 ${result.session_id} · hash ${result.session_hash.slice(0, 12)}…`, "success");
  ...
  async function bootstrapForSeed(seed) {
    if (!state.bootstraps.has(seed)) state.bootstraps.set(seed, await fetchJson(`/api/bootstrap?seed=${seed}`));
  ...
  async function switchSeed(seed) {
    ...
    ui.seedTitle.textContent = `Seed ${seed}`;
    const url = new URL(window.location.href);
    url.search = "";
  ...
  async function refreshTargets() {
    try {
      state.targetIndex = await fetchJson(`/api/sessions?seed=${state.seed}`);
      ui.targetSelect.replaceChildren();
      if (!state.targetIndex.targets.length) {
        const option = document.createElement("option");
        option.value = "";
        option.textContent = "暂无 target";
        ...
        state.targetIndex.targets.forEach((target) => {
          ...
          option.textContent = `${target.session_id} · ΔRMS ${target.metrics.control_point_rms_delta}`;
          ui.targetSelect.appendChild(option);
        });
        ui.compare.disabled = false;
      }
      const aggregate = state.targetIndex.aggregate;
      ui.targetStats.textContent = `${aggregate.target_count} 个 target；控制点 ΔRMS 均值 ${aggregate.control_point_rms_delta_mean}，范围 ${aggregate.control_point_rms_delta_range.join("–")}；mount MAE 均值 ${aggregate.mount_mean_absolute_delta_mean}`;
    } catch (error) {
  ...
  async function compareTarget() {
    ...
    if (!sessionId) return;
    try {
      const target = await fetchJson(`/api/sessions/${encodeURIComponent(sessionId)}`);
      state.targetComparison = compareBranches(state.branches, target.branches);
      const result = state.targetComparison;
      ui.targetStats.textContent = `对比 ${sessionId}：共享 ${result.shared}；当前独有 ${result.currentOnly}；target 独有 ${result.targetOnly}；控制点 RMS ${result.rms.toFixed(4)}；mount MAE ${result.mountMae.toFixed(6)}；状态差异 ${result.statusDifferences}`;
    } catch (error) {
      ui.targetStats.textContent = error.message;
    }
  }
  ...
  function setViewMode(mode) {
    ...
    if (mode === "unit" && !state.selectedId) setMessage("Unit 视图需要先选择一条分支。", "error");
    render();
  }
  ...
  async function initialize() {
    ...
    let session = null;
    let seed = Number(query.get("seed") || window.EDITOR_SEED);
    if (requestedSession) {
      session = await fetchJson(`/api/sessions/${encodeURIComponent(requestedSession)}`);
    ...
    if (session) setMessage(`已重载 ${requestedSession} · hash ${session.session_hash.slice(0, 12)}…`, "success");
    await refreshTargets();
    render();
  }
