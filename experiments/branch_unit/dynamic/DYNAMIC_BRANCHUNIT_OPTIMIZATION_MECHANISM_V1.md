...
| 批量 SVG 导出 | `export_batch_svgs.py` |
...
批量求解修复（2026-08-17）：500 案例首跑出现两类真实失败，均已定位并修复，
并保持 15 个冻结案例逐字节一致：

- `proto_sw_1_3` ×12：上游集合选择净空阈值 0.03125 小于下游 Stage5 完整 Unit
  净空 0.041459216，选出的 lane 对下游不可消费。修复为“下游净空传播”：批量
  路径把 Stage5 要求的净空传入上游；若首选集合违反，按该阈值重解并额外过滤
  承花近贴候选，重解事实记录在 solver 的 `downstream_clearance_resolve`；
- `proto_sw_3_1` ×2：上游求解器对“不存在 7 元可行集”的基数在精确搜索达到
  节点上限时误抛 `l1_set_search_exhausted`。修复为两个求解器改进：精确搜索前
  增加按 branch_seed 确定性排队的种子化救援见证者搜索；节点上限耗尽不再直接
  抛错，而是记录 `exact_search_exhausted_no_witness` 并继续，整体无任何可行
  基数时仍如实报错。

修复证据：冻结 15/15 案例 plan 逐字节一致；14 个失败 seed 修复后全链
（上游→Stage4→Stage5→机械检查）14/14 通过；批量断点续跑补齐后
`dynamic_branch_batch_v1` 共 500/500 成功，每原型 9/9 格子覆盖，选中曲线
交叉 0、净空违规 0；历史失败保留在 `failures_history.json` 并标注救回方式。
