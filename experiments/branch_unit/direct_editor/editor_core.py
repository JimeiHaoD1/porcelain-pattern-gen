"""Core geometry, validation, and persistence for the direct branch editor.

The module deliberately owns the filesystem allowlist and re-validates geometry
received from the browser.  It does not import or mutate the branch generator.
"""
...
SEED = 4101
ALLOWED_SEEDS = (4101, 4102, 4103)
ROOT = Path(__file__).resolve().parents[3]
RUN_DIR = ROOT / "artifacts" / "runs" / "_scratch_whole_local_true_lateral_n12_v23"
RECORD_PATH = RUN_DIR / "record_4101.json"
REFERENCE_PNG_PATH = RUN_DIR / "seed_4101.png"
PROFILE_PATH = (
    ROOT
    / "artifacts"
    / "runs"
...
SESSION_ROOT = ROOT / "artifacts" / "editor_sessions"
...
def record_path(seed: int) -> Path:
    if seed not in ALLOWED_SEEDS:
        raise EditorValidationError("只允许 seed 4101、4102、4103")
    return RUN_DIR / f"record_{seed}.json"


def reference_png_path(seed: int) -> Path:
    if seed not in ALLOWED_SEEDS:
        raise EditorValidationError("只允许 seed 4101、4102、4103")
    return RUN_DIR / f"seed_{seed}.png"


def load_source(seed: int = SEED) -> dict[str, Any]:
    """Load an allowlisted source and normalize it for browser bootstrap."""

    selected_record = record_path(seed)
    selected_reference = reference_png_path(seed)
    for path in (selected_record, PROFILE_PATH, selected_reference):
        if not path.is_file():
            raise EditorError(f"固定输入不存在: {path}")

    record = _read_json(selected_record)
    profile = _read_json(PROFILE_PATH)
    try:
        result = record["result"]
        source_curves = result["curves"]
        arc_samples = profile["backbone"]["arc_samples"]
        canvas = profile["canvas"]
        repeat_x = profile["repeat_x_range"]
        flowers = profile["flowers"]
    except (KeyError, TypeError) as exc:
        raise EditorError(f"固定输入缺少必要字段: {exc}") from exc

    branches: list[dict[str, Any]] = []
    seen: set[str] = set()
    for index, wrapper in enumerate(source_curves):
        try:
            curve_id = str(wrapper["curve_id"])
            curve = wrapper["curve"]
            parent_id = str(wrapper["parent_id"])
            level = int(wrapper["level"])
        except (KeyError, TypeError, ValueError) as exc:
            raise EditorError(f"record 曲线 {index} 结构无效: {exc}") from exc
        if curve_id in seen:
            raise EditorError(f"record 含重复 curve_id: {curve_id}")
        seen.add(curve_id)
        cubics = _cubics(curve.get("cubics"), f"record.{curve_id}.cubics")
        branches.append(
            {
                "curve_id": curve_id,
                "parent_id": parent_id,
                "level": level,
                "role": str(curve.get("role", "")),
                "kind": str(wrapper.get("kind", "")),
                "source": str(wrapper.get("source", "")),
                "target_flower_id": wrapper.get("target_flower_id"),
                "mount_fraction": float(curve["mount_fraction"]),
                "original_mount_fraction": float(curve["mount_fraction"]),
                "width": float(curve["width"]),
                "status": "required",
                "original_cubics": copy.deepcopy(cubics),
                "edited_cubics": copy.deepcopy(cubics),
                "constraints": {
    ...
    _validate_parent_graph(branches)
    backbone = [_point(row["point"], "profile.backbone.arc_samples.point") for row in arc_samples]
    flower_rows = [
        {
            "flower_id": str(row["flower_id"]),
            "center": _point(row["center"], "profile.flower.center"),
            "rx": float(row["rx"]),
            "ry": float(row["ry"]),
        }
        for row in flowers
    ...
    return {
        ...
        "source": {
            "run_dir": str(RUN_DIR),
            "seed": seed,
            "record_file": selected_record.name,
            "record_path": str(selected_record),
            "source_record_sha256": file_sha256(selected_record),
            "source_geometry_hash": str(result["geometry_hash"]),
            "profile_file": str(PROFILE_PATH),
            "profile_sha256": file_sha256(PROFILE_PATH),
            "reference_png": str(selected_reference),
        },
        "canvas": {
            "width": int(canvas["width"]),
            "height": int(canvas["height"]),
            "active_repeat_x": [float(repeat_x[0]), float(repeat_x[1])],
            "strict_edges": ["top", "bottom"],
        },
        "backbone": {"points": backbone, "locked": True},
        "flowers": flower_rows,
        "branches": branches,
        "topology": copy.deepcopy(result.get("topology", {})),
        "stroke_model": copy.deepcopy(result.get("stroke_model", {})),
        "forbidden_regions": [],
        "warnings": [],
        "edit_summary": {},
    }
...
def validate_session(payload: Any, source: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not isinstance(received_branches, list) or not 1 <= len(received_branches) <= MAX_BRANCHES:
        raise EditorValidationError(f"session.branches 必须包含 1 到 {MAX_BRANCHES} 条分支")
    ...
    for row in received_branches:
        ...
        if expected is not None:
            ...
            if stable_hash({"cubics": original}) != stable_hash(
                {"cubics": expected["original_cubics"]}
            ):
    ...
    result["session_hash"] = stable_hash(result)
    return result
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="{view_x:.6f} 0 {view_width:.6f} {height:.6f}">
  <metadata>{html.escape(_canonical_json({"schema": SCHEMA, "session_hash": session["session_hash"]}))}</metadata>
  <style>
...
def save_session(payload: Any, session_id: str | None = None) -> tuple[str, dict[str, Any], Path]:
    """Validate and atomically persist session.json plus edited.svg."""

    if not isinstance(payload, dict) or not isinstance(payload.get("source"), dict):
        raise EditorValidationError("session.source 缺失")
    try:
        seed = int(payload["source"]["seed"])
    except (TypeError, ValueError) as exc:
        raise EditorValidationError("session.source.seed 无效") from exc
    selected_record = record_path(seed)
    source_hash_before = file_sha256(selected_record)
    source = load_source(seed)
    if source_hash_before != source["source"]["source_record_sha256"]:
        raise EditorError("source record 在读取期间发生变化")
    session = validate_session(payload, source)
    digest = session["session_hash"]
    if session_id is None:
        timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ")
        session_id = f"seed{seed}_{timestamp}_{digest[:10]}"
    if not SESSION_ID_RE.fullmatch(session_id):
        raise EditorValidationError("非法 session_id")

    SESSION_ROOT.mkdir(parents=True, exist_ok=True)
    target = SESSION_ROOT / session_id
    if target.exists():
        raise EditorValidationError(f"session 已存在: {session_id}")
    temporary = Path(tempfile.mkdtemp(prefix=".tmp_direct_editor_", dir=SESSION_ROOT))
    try:
        _write_text(temporary / "session.json", json.dumps(session, ensure_ascii=False, indent=2) + "\n")
        _write_text(temporary / "edited.svg", render_edited_svg(session, source))
        if file_sha256(selected_record) != source_hash_before:
            raise EditorError("保存期间 source record 发生变化，已取消保存")
        temporary.replace(target)
    except Exception:
        shutil.rmtree(temporary, ignore_errors=True)
        raise
    return session_id, session, target
...
def load_session(session_id: str) -> dict[str, Any]:
    ...
    expected_hash = stored.get("session_hash")
    if not isinstance(expected_hash, str) or stable_hash(stored) != expected_hash:
        raise EditorValidationError("session.json 内容 hash 校验失败")
    try:
        seed = int(stored.get("source", {}).get("seed"))
    except (TypeError, ValueError) as exc:
        raise EditorValidationError("session.source.seed 无效") from exc
    legacy = any(
        "constraints_changed" not in delta
        for delta in (stored.get("edit_summary", {}).get("curve_deltas", {}) or {}).values()
    )
    normalized = validate_session(stored, load_source(seed))
    if normalized["session_hash"] != expected_hash and not legacy:
        raise EditorValidationError("session 重放后 hash 不一致")
    return normalized
...
def list_sessions(seed: int) -> dict[str, Any]:
    source = load_source(seed)
    targets: list[dict[str, Any]] = []
    if SESSION_ROOT.is_dir():
        for directory in sorted(SESSION_ROOT.iterdir(), key=lambda path: path.name, reverse=True):
            if not directory.is_dir() or not SESSION_ID_RE.fullmatch(directory.name):
                continue
            try:
                session = load_session(directory.name)
            except (EditorError, FileNotFoundError, OSError):
                continue
            if int(session["source"]["seed"]) != seed:
                continue
            targets.append(
                {
                    "session_id": directory.name,
                    "session_hash": session["session_hash"],
                    "metrics": _session_metrics(session, source),
                }
            )
    rms_values = [target["metrics"]["control_point_rms_delta"] for target in targets]
    mount_values = [target["metrics"]["mount_mean_absolute_delta"] for target in targets]
    aggregate = {
        "target_count": len(targets),
        "control_point_rms_delta_mean": round(sum(rms_values) / len(rms_values), 9)
        if rms_values
        else 0.0,
        "control_point_rms_delta_range": [min(rms_values), max(rms_values)] if rms_values else [0.0, 0.0],
        "mount_mean_absolute_delta_mean": round(sum(mount_values) / len(mount_values), 12)
        if mount_values
        else 0.0,
        "mount_mean_absolute_delta_range": [min(mount_values), max(mount_values)]
        if mount_values
        else [0.0, 0.0],
    }
    return {"seed": seed, "targets": targets, "aggregate": aggregate}
