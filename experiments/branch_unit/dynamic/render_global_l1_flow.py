...
def _bounds(analysis: Mapping[str, Any], repeat_count: int) -> tuple[float, float, float, float]:
    height = float(analysis["coordinate_system"]["canvas_bounds"][3])
    if repeat_count == 1:
        return -0.34, 1.34, 0.0, height
    return -1.08, 2.08, 0.0, height
...
def _xy(
    ...
    x_min, x_max, y_min, y_max = _bounds(analysis, repeat_count)
...
def _lane_points(lane: Mapping[str, Any], offset: float) -> list[tuple[float, float]]:
    points: list[tuple[float, float]] = []
    for segment_index, segment in enumerate(lane["segments"]):
        sampled = _sample_cubic(segment)
        if segment_index:
            sampled = sampled[1:]
        points.extend((point[0] + offset, point[1]) for point in sampled)
    return points
...
def render_png(
    ...
    *,
    ...
    for x_value in boundary_values:
        draw.line(
            [_xy(analysis, (x_value, 0.0), repeat_count), _xy(analysis, (x_value, height), repeat_count)],
            fill=COLORS["grid"],
            width=2,
        )
    ...
    for offset in _offsets(repeat_count):
        backbone_points = [
            _xy(analysis, (float(point[0]) + offset, float(point[1])), repeat_count)
            for point in backbone
        ]
        draw.line(backbone_points, fill=COLORS["backbone"], width=5, joint="curve")

        for flower in analysis["flowers"]:
            center = (float(flower["center"][0]) + offset, float(flower["center"][1]))
            center_px = _xy(analysis, center, repeat_count)
            rx_px = abs(
                _xy(
                    analysis,
                    (center[0] + float(flower["rx"]), center[1]),
                    repeat_count,
                )[0]
                - center_px[0]
            )
    ...
    for lane in plan["lanes"]:
        color = _color(lane)
        for offset in _offsets(repeat_count):
            world = _lane_points(lane, offset)
            screen = [_xy(analysis, point, repeat_count) for point in world]
    ...
    draw.text(
        (48, 858),
        f'L1={len(plan["lanes"])}  {role_counts}  ·  状态：待人工视觉验收',
        font=_font(15),
        fill=COLORS["muted"],
    )
    image.save(output)
...
def _svg_path(lane: Mapping[str, Any], analysis: Mapping[str, Any], repeat_count: int, offset: float) -> str:
    commands: list[str] = []
    for segment_index, segment in enumerate(lane["segments"]):
        points = {
            key: _xy(
                analysis,
                (float(value[0]) + offset, float(value[1])),
                repeat_count,
            )
...
def render_svg(
    ...
    *,
    ...
    for x_value in boundaries:
        p0 = _xy(analysis, (x_value, 0.0), repeat_count)
        p1 = _xy(analysis, (x_value, height), repeat_count)
        rows.append(
    ...
    for offset in _offsets(repeat_count):
        ...
        for flower in analysis["flowers"]:
            center = _xy(
                analysis,
                (float(flower["center"][0]) + offset, float(flower["center"][1])),
                repeat_count,
            )
            rx = abs(
                _xy(
                    analysis,
                    (
                        float(flower["center"][0]) + offset + float(flower["rx"]),
    ...
    for lane in plan["lanes"]:
        color = _color(lane)
        for offset in _offsets(repeat_count):
            path = _svg_path(lane, analysis, repeat_count, offset)
            rows.append(
