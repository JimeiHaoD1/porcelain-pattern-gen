...
当前视觉关口：5D 已实现，`VISUAL_REVIEW_PENDING`
...
当前未知：这些机械合法结果是否达到用户要求的视觉疏密质量。未确认前 5D 不能写成视觉通过。
...
| 5D | 普通 L1 simple/medium/rich | 已实现、全原型图已生成 | 待确认 |
...
必须以用户查看 5D 正式图后的意见判断。

若 5D 被否定，只返工密度状态和普通 L1 集合选择，不回退已确认的 5B、5C 和承花机制。
