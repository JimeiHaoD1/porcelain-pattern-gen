...
def _pair_metrics(
    ...
    if curve_geometry_profile is not None:
        distribution = curve_geometry_profile["composition_pair_geometry"][
            "parallel_co_travel_score"
        ]
        co_travel_soft_limit = float(distribution["q75"])
        co_travel_hard_limit = float(distribution["q95"])
        if co_travel > co_travel_hard_limit + 1e-9:
            return False, 0.0, {
                "root_gap": root_gap,
        ...
        co_travel_penalty = min(
            1.0,
            max(0.0, co_travel - co_travel_soft_limit)
            / max(co_travel_hard_limit - co_travel_soft_limit, 0.01),
        )
